...
interface FormData {
  ...
  const buyCourseAction = async (courseId:string): Promise<any | ErrorResponse> => {
    
  let cookValue = cookies().get("user-connect")?.value;
    try {
      const response = await axios.post(`${process.env.B_HOST}/user/enroll`, {course_id: courseId}, {
...
export default buyCourseAction